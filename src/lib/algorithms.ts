...
export const updateSimilarityFor = async function(userId: string){
  ...
  const userRatedItemIds = await client.sunion(Key.userLikedSet(userId), Key.userDislikedSet(userId))
  ...
  if (userRatedItemIds.length > 0){
    ...
    itemLikeDislikeKeys = _(userRatedItemIds).map(function(itemId, key){
...
export const updateRecommendationsFor = async function(userId: string){
  ...
  let setsToUnion: string[] = [];
  let scoreMap: [number, string][] = [];
  ...
  const mostSimilarUserIds = await client.zrevrange(similarityZSet, 0, config.nearestNeighbors-1)
  const leastSimilarUserIds = await client.zrange(similarityZSet, 0, config.nearestNeighbors-1)
  ...
  if (setsToUnion.length > 0){
    ...
    // await client.sunionstore(setsToUnion) //TODO: 確認
    await client.sunionstore(tempAllLikedSet, ...setsToUnion)
    const notYetRatedItems = await client.sdiff(tempAllLikedSet, Key.userLikedSet(userId), Key.userDislikedSet(userId))

    await pMap(notYetRatedItems, async function(itemId, callback){
    ...
    await pMap(scoreMap, async function(scorePair){
    ...
    await client.zremrangebyrank(recommendedZSet, 0, length-config.numOfRecsStore-1)    
  }
};
