...
function liked(userId, itemId, callback) {
...
function disliked(userId, itemId, callback) {
...
var input = {
  liked: function(userId, itemId, callback){
    liked(userId, itemId, function() {
      updateSequence(userId, itemId, callback);
    });
  },
  likedMultiple: function(userId, items, callback){
    var itemProcessed = 0;
  ...
  dislikedMultiple: function(userId, items, callback){
    var itemProcessed = 0;
    items.forEach(function(itemId) {
      disliked(userId, itemId, function() {
        itemProcessed++;
        if (itemProcessed === items.length) {
          updateSequence(userId, itemId, callback);
        }
