...
var input = {
  ...
  unliked: function(userId, itemId, callback){
    client.sismember([config.className, itemId, 'liked'].join(":"), userId, function(err, results){
      if (results > 0){
        client.zincrby([config.className, 'mostLiked'].join(":"), -1, itemId);
      }
      client.srem([config.className, userId,'liked'].join(':'), itemId, function(err){
        client.srem([config.className, itemId, 'liked'].join(':'), userId, function(err){
          updateSequence(userId, itemId, callback);
        });
      });
    });
  },
  undisliked: function(userId, itemId, callback){
    client.sismember([config.className, itemId, 'disliked'].join(":"), userId, function(err, results){
      if (results > 0){
        client.zincrby([config.className, 'mostDisliked'].join(":"), -1, itemId);
      }
      client.srem([config.className, userId, 'disliked'].join(':'), itemId, function(err){
        client.srem([config.className, itemId, 'disliked'].join(':'), userId, function(err){
          updateSequence(userId, itemId, callback);
        });
      });
    });
