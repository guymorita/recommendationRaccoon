...
const stat = {
  ...
  likedBy: function(itemId){
    return new Promise((resolve, reject) => {
      client.smembersAsync([CLASSNAME,itemId,'itemliked'].join(':')).then((results) => {
        resolve(results);
      });
    });
  },
  likedCount: function(itemId){
    return new Promise((resolve, reject) => {
      client.scardAsync([CLASSNAME,itemId, 'itemliked'].join(':')).then((results) => {
        resolve(results);
      });
    });
  },
  ...
  allWatchedFor: function(userId){
    return new Promise((resolve, reject) => {
      client.sunionAsync([CLASSNAME, 'user', userId, 'liked'].join(":"), [config.className, 'user', userId, 'disliked'].join(":")).then((results) => {
        resolve(results);
      });
    });
  }
