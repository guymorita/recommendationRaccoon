...
const jaccardCoefficient = function(userId1, userId2, callback){
  ...
  const user1LikedSet = [config.className,userId1,'userliked'].join(":"),
    user1DislikedSet = [config.className,userId1,'userdisliked'].join(":"),
    user2LikedSet = [config.className,userId2,'userliked'].join(":"),
    user2DislikedSet = [config.className,userId2,'userdisliked'].join(":");
...
exports.updateSimilarityFor = function(userId, cb){
  ...
  const similaritySet = [config.className,userId,'similaritySet'].join(":");
  // creating a combined set with the all of a users likes and dislikes
  client.sunion([config.className,userId,'userliked'].join(":"),[config.className,userId,'userdisliked'].join(":"), function(err, userRatedItemIds){
    ...
    if (userRatedItemIds.length > 0){
      ...
      itemLikeDislikeKeys = _.map(userRatedItemIds, function(itemId, key){
        ...
        itemLiked = [config.className, itemId, 'itemliked'].join(":");
        // key for the item being disliked
        itemDisliked = [config.className, itemId, 'itemdisliked'].join(":");
...
exports.predictFor = function(userId, itemId, callback){
  ...
  let prediction = 0.0;
  const similaritySet = [config.className, userId, 'similaritySet'].join(':');
  const likedBySet = [config.className, itemId, 'itemliked'].join(':');
  const dislikedBySet = [config.className, itemId, 'itemdisliked'].join(':');
...
exports.updateRecommendationsFor = function(userId, cb){
  ...
  const tempSet = [config.className, userId, 'tempSet'].join(":");
  const tempDiffSet = [config.className, userId, 'tempDiffSet'].join(":");
  const similaritySet = [config.className, userId, 'similaritySet'].join(":");
  const recommendedSet = [config.className, userId, 'recommendedSet'].join(":");
  ...
  client.zrevrange(similaritySet, 0, config.nearestNeighbors-1, function(err, mostSimilarUserIds){
    ...
    client.zrange(similaritySet, 0, config.nearestNeighbors-1, function(err, leastSimilarUserIds){
      ...
      _.each(mostSimilarUserIds, function(id, key){
        setsToUnion.push([config.className,id,'userliked'].join(":"));
      });
      ...
      if (config.factorLeastSimilarLeastLiked){
        _.each(leastSimilarUserIds, function(id, key){
          setsToUnion.push([config.className,id,'userdisliked'].join(":"));
        });
      }
      ...
      if (setsToUnion.length > 0){
        ...
        async.each(setsToUnion,
          ...
          function(err){
            ...
            client.sdiff(tempSet, [config.className,userId,'userliked'].join(":"), [config.className,userId,'userdisliked'].join(":"), function(err, notYetRatedItems){
...
exports.updateWilsonScore = function(itemId, callback){
  ...
  const scoreBoard = [config.className, 'scoreBoard'].join(":");
  const likedBySet = [config.className, itemId, 'itemliked'].join(':');
  const dislikedBySet = [config.className, itemId, 'itemdisliked'].join(':');
