...
const jaccardCoefficient = function(userId1, userId2, callback){
  ...
  const user1LikedSet = [config.className,userId1,'liked'].join(":"),
    user1DislikedSet = [config.className,userId1,'disliked'].join(":"),
    user2LikedSet = [config.className,userId2,'liked'].join(":"),
    user2DislikedSet = [config.className,userId2,'disliked'].join(":");
...
exports.predictFor = function(userId, itemId, callback){
  ...
  let prediction = 0.0;
  const similaritySet = [config.className, userId, 'similaritySet'].join(':');
  const likedBySet = [config.className, itemId, 'liked'].join(':');
  const dislikedBySet = [config.className, itemId, 'disliked'].join(':');
